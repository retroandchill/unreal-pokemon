...
public class UGameDataEntryPbsSerializer : UCSGameDataEntrySerializer
{
    protected override bool Supports(TSubclassOf<UAssetGameDataRepository> repositoryClass)
    {
        ...
        try
        {
            var matchingType = repositoryClass.DefaultObject.GetType()
                .GetInterfaces()
                .Where(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IGameDataRepository<>))
                .Select(i => i.GetGenericArguments()[0])
                .SingleOrDefault(t =>
                {
                    var method = typeof(UGameDataEntryPbsSerializer).GetMethod(nameof(MatchesScriptStruct),
                        BindingFlags.NonPublic | BindingFlags.Static)!.MakeGenericMethod(t);
                    return (bool)method.Invoke(null, [entryStruct])!;
                });

            return matchingType is not null && PbsSerializer.Instance.HasSerializerFor(matchingType);
        }
        catch (Exception e)
        {
    ...
    protected override bool Serialize(string filePath, UAssetGameDataRepository repository, out string errorMessage)
    {
        try
        {
            ...
            var matchingType = repository.GetType()
                .GetInterfaces()
                .Where(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IGameDataRepository<>))
                .Select(i => i.GetGenericArguments()[0])
                .Single(t =>
            ...
            var serializeMethod = typeof(UGameDataEntryPbsSerializer).GetMethod(nameof(SerializeInternal),
                BindingFlags.NonPublic | BindingFlags.Static)!.MakeGenericMethod(matchingType);
    ...
    protected override bool Deserialize(string filePath, UAssetGameDataRepository repository, out string errorMessage)
    {
        try
        {
            ...
            var serializeMethod = typeof(UGameDataEntryPbsSerializer).GetMethod(nameof(DeserializeInternal),
                BindingFlags.NonPublic | BindingFlags.Static)!.MakeGenericMethod(matchingType);
            serializeMethod.Invoke(this, [filePath, repository]);
            errorMessage = string.Empty;
            return true;
    ...
    private static bool MatchesScriptStruct<T>(UScriptStruct scriptStruct) where T : MarshalledStruct<T>
    {
        ...
        where T : struct, IGameDataEntry, MarshalledStruct<T>
    {
        ...
        if (repository.GetType().GetProperty("DataEntries", 
                BindingFlags.NonPublic | BindingFlags.Instance)!.GetValue(repository) is not IList<T> dataEntries)
        {
