...
UE5Coro::TCoroutine<> USwitchAction::SwapWithTarget(const TScriptInterface<IBattler> &SwappingFrom,
                                                    ...
                                                    IPokemonCoroutineDispatcher &Dispatcher)
{
    ...
    auto &SendOutFormat =
        SwapTarget->IsOwnedByPlayer() ? Messages->PlayerSendOutMessage : Messages->OpponentSendOutMessage;
    co_await Dispatcher.DisplayMessage(FText::FormatNamed(SendOutFormat, "Pkmn", SwapTarget->GetNickname(), "Trainer",
                                                          OwningTrainer->GetFullTrainerName()));
