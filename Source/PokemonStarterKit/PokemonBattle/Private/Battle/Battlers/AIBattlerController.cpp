...
void UAIBattlerController::ActionSelection(const TScriptInterface<IBattler> &Battler) const
{
    ...
    AsyncTask(ENamedThreads::AnyThread, [this, Battler] {
        ...
        // TODO: Right now we're just getting a proof of concept for the battle system for now, but eventually we will
        // want this class to call to a series of additional child objects that represent the checks that can be used.
        // It may be best to store a map or a list in a Data Asset that has all of the checks that would be applied and
        // the minimum skill level needed to add those checks. For now though, just choose a random usable move and
        // struggle if there are no such moves.
