...
#define AUTOMATION_ASSERT(Expression) if (!(Expression)) return false
...
#define MakeTestableBP(BPName, GraphName)												\
	...
	auto GraphName = BPName ? TUniquePtr<UEdGraph, FGraphDeleter>(FBlueprintEditorUtils::FindEventGraph( BPName.Get() )) : nullptr;	
