...
bool TestIsGameDataIDValid_NodeTile::RunTest(const FString &Parameters) {
    ...
    AUTOMATION_ASSERT(TestEqual(TEXT("Node title should contain superclass!"),
                                TestNode->GetNodeTitle(MenuTitle).ToString(), TEXT("Is Growth Rate Data ID Valid?")));
...
bool TestIsGameDataIDValid_MenuActions::RunTest(const FString &Parameters) {
    ...
    AUTOMATION_ASSERT(
        TestTrue(TEXT("Node should have actions!"), Actions.Contains(UK2Node_IsGameDataIDValid::StaticClass())));
