...
void ATestWindowskinThumbnailRenderer::TestRun() {
	...
	TEST_ASSERT(AssertTrue(Renderer->GetThumbnailRenderFrequency(InvalidWindowskin.Get()) == EThumbnailRenderFrequency::Realtime,
		TEXT("Realtime Thumbnails")));
	TEST_ASSERT(AssertTrue(Renderer->GetThumbnailRenderFrequency(ValidWindowskin) == EThumbnailRenderFrequency::Realtime,
		TEXT("Realtime Thumbnails")));
