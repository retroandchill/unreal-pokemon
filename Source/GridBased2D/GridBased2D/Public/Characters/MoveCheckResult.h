...
struct GRIDBASED2D_API FMoveCheckResult {
     ...
     */
    template <std::size_t Index> std::tuple_element_t<Index, FMoveCheckResult> &get() {
...
namespace std {
template <> struct tuple_size<FMoveCheckResult> : integral_constant<size_t, 2> {};

template <> struct tuple_element<0, FMoveCheckResult> {
    using type = bool;
};

template <> struct tuple_element<1, FMoveCheckResult> {
    using type = TArray<TScriptInterface<IInteractable>>;
};

template <size_t Index> struct tuple_element<Index, FMoveCheckResult> : conditional<Index == 0, std::string, int> {
