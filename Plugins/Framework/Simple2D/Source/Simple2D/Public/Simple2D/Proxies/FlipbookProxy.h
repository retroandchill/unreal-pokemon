...
namespace Simple2D
{
    ...
    template <typename T>
    concept Flipbook =
        std::derived_from<T, UObject> && requires(const T *Object, int32 Index, float Time, bool bClampToEnds) {
        ...
        } && requires(T *Object) { Object->InvalidateCachedData(); };
    ...
    class FFlipbookProxy : public FGCObject
    {
      ...
      private:
        ...
        {
            static FVTable VTable = {.GetNumFrames = TVTableImpl<T>::GetNumFrames,
