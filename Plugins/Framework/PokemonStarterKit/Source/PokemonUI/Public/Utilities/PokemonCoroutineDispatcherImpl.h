...
class POKEMONUI_API UPokemonCoroutineDispatcherImpl : public UObject, public IPokemonCoroutineDispatcher {
  ...
  public:
    ...
    TMultiCoroutine<int32, FName> DisplayMessageWithChoices(FText Message, const TArray<FText> &Choices,
                                                            FForceLatentCoroutine = {}) const override;

    UE5Coro::TCoroutine<bool> DisplayConfirmPrompt(FText Message,
    ...
    UE5Coro::TCoroutine<bool> PromptReplaceMove(const TScriptInterface<IPokemon> &Pokemon, FMoveHandle Move,
                                                FForceLatentCoroutine = {}) const override;

    UE5Coro::TCoroutine<bool> GiveItemToPokemon(const FItemHandle &Item, const TScriptInterface<IPokemon> Pokemon,
                                                int PokemonIndex, FForceLatentCoroutine = {}) const override;

    UE5Coro::TCoroutine<bool> TakeItemFromPokemon(const TScriptInterface<IPokemon> &Pokemon,
                                                  FForceLatentCoroutine = {}) const override;

    UE5Coro::TCoroutine<bool> LearnMove(const TScriptInterface<IPokemon> &Pokemon, FMoveHandle Move,
                                        FForceLatentCoroutine = {}) const override;

    UE5Coro::TCoroutine<> ProcessLevelUp(const TScriptInterface<IPokemon> &Pokemon,
