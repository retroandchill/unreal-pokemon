...
void STileSelector::Construct(const FArguments &InArgs) {
    ...
    auto PlatformComboBoxContentText = [this]() -> FText {
        ...
        return SelectedTile.IsValid() ? FText::FromString(*SelectedTile.ToString())
    ...
    ChildSlot[SNew(SVerticalBox) +
              SVerticalBox::Slot()
                  .VAlign(VAlign_Top)
                  .HAlign(HAlign_Fill)
                  .AutoHeight()[SAssignNew(TileComboBox, SComboBox<FName>)
                                    ...
                                    .OnSelectionChanged_Raw(this, &STileSelector::OnTileSelectionChanged)
                                        [SNew(STextBlock).Text_Lambda(PlatformComboBoxContentText)]] +
              SVerticalBox::Slot()
                  .VAlign(VAlign_Fill)
                  .HAlign(HAlign_Fill)
                  .FillHeight(1.f)[SAssignNew(SelectorViewport, STileSelectorViewport, SelectorClient)]];
}
