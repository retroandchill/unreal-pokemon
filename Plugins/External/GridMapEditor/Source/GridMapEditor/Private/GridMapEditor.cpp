...
void FGridMapEditorModule::StartupModule() {
    ...
    RegisterCustomPropertyTypeLayout("GridMapTileBitset", FOnGetPropertyTypeCustomizationInstance::CreateStatic(
                                                              &FTileBitsetCustomization::MakeInstance));

    // Custom thumbnail renderes
    UThumbnailManager::Get().RegisterCustomRenderer(UGridMapTileSet::StaticClass(),
                                                    UTileSet_ThumbnailRenderer::StaticClass());

    // Register asset types
    IAssetTools &AssetTools = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools").Get();
    GridMapAssetCategory = AssetTools.RegisterAdvancedAssetCategory(FName(TEXT("GridMapEditor")),
                                                                    LOCTEXT("GridMapAssetCategory", "Grid Map"));
    RegisterAssetTypeAction(AssetTools, MakeShareable(new FGridMapTileSetAssetTypeActions(GridMapAssetCategory)));

    // Modes
    FEditorModeRegistry::Get().RegisterMode<FGridMapEditorMode>(
        FGridMapEditorMode::EM_GridMapEditorModeId, LOCTEXT("GridMapEditorModeName", "GridMapEditorMode"),
        FSlateIcon("GridMapStyle", "GridMapEditor.Tab", "GridMapEditor.Tab.Small"), true);
