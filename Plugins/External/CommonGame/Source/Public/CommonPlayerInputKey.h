...
UENUM(BlueprintType)
enum class ECommonKeybindForcedHoldStatus : uint8 { NoForcedHold, ForcedHold, NeverShowHold };
...
class COMMONGAME_API UCommonPlayerInputKey : public UCommonUserWidget {
  ...
  public:
    ...
    /** Key this widget is bound to set directly in blueprint. Used when we want to reference a specific key instead of
     * an action. */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Keybind Widget")
    ...
    bool IsHoldKeybind() const {
        return bIsHoldKeybind;
    }

    UFUNCTION()
    bool IsBoundKeyValid() const {
        return BoundKey.IsValid();
    }
