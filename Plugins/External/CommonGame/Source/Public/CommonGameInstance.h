...
class COMMONGAME_API UCommonGameInstance : public UGameInstance {
  ...
  public:
     ...
     *   Something requests the user to join a specific session (for example, a platform overlay via
     * OnUserRequestedSession). This request is handled in SetRequestedSession. Check if we can join the requested
     * session immediately (CanJoinRequestedSession).  If we can, join the requested session (JoinRequestedSession) If
     * not, cache the requested session and instruct the game to get into a state where the session can be joined
     * (ResetGameAndJoinRequestedSession)
     */
    /** Handles user accepting a session invite from an external source (for example, a platform overlay). Intended to
    ...
    UCommonSession_SearchResult *GetRequestedSession() const {
