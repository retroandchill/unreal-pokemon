...
UENUM(BlueprintType)
enum class ECommonSessionOnlineMode : uint8 { Offline, LAN, Online };
...
class COMMONUSER_API UCommonSession_SearchResult : public UObject {
  ...
  public:
    ...
    /** The maximum number of publicly available connections that could be available, including already filled
     * connections */
    UFUNCTION(BlueprintPure, Category = Sessions)
...
class COMMONUSER_API UCommonSession_SearchSessionRequest : public UObject {
  ...
  public:
    ...
    /** True if this request should look for player-hosted lobbies if they are available, false will only search for
     * registered server sessions */
    UPROPERTY(BlueprintReadWrite, Category = Session)
    bool bUseLobbies;
...
/**
 * Event triggered when the local user has requested to join a session from an external source, for example from a
 * platform overlay. Generally, the game should transition the player into the session.
 * @param LocalPlatformUserId the local user id that accepted the invitation. This is a platform user id because the
...
/**
 * Event triggered when a session join has completed, after joining the underlying session and before traveling to the
 * server if it was successful. The event parameters indicate if this was successful, or if there was an error that will
 * stop it from traveling.
...
/**
 * Event triggered when a session join has completed, after resolving the connect string and prior to the client
 * traveling.
 ...
 * This should not be used for online functionality (use OnCreateSessionComplete or OnJoinSessionComplete for those) but
 * for features such as rich presence
 */
UENUM(BlueprintType)
enum class ECommonSessionInformationState : uint8 { OutOfGame, Matchmaking, InGame };
...
class COMMONUSER_API UCommonSessionSubsystem : public UGameInstanceSubsystem {
  ...
  public:
    ...
    /** Creates a session search object with default options to look for default online games, this can be modified
     * after creation */
    UFUNCTION(BlueprintCallable, Category = Session)
    virtual UCommonSession_SearchSessionRequest *CreateOnlineSearchSessionRequest();

    /** Creates a new online game using the session request information, if successful this will start a hard map
     * transfer */
    UFUNCTION(BlueprintCallable, Category = Session)
  ...
  protected:
    // Functions called during the process of creating or joining a session, these can be overidden for game-specific
    // behavior

    /** Called to fill in a session request from quick play host settings, can be overridden for game-specific behavior
     */
    virtual TSharedRef<FCommonOnlineSearchSettings>
    CreateQuickPlaySearchSettings(UCommonSession_HostSessionRequest *Request,
                                  UCommonSession_SearchSessionRequest *QuickPlayRequest);
  ...
  protected:
    ...
    CreateQuickPlaySearchSettingsOSSv1(UCommonSession_HostSessionRequest *Request,
                                       UCommonSession_SearchSessionRequest *QuickPlayRequest);
    ...
    TSharedRef<FCommonOnlineSearchSettings>
    CreateQuickPlaySearchSettingsOSSv2(UCommonSession_HostSessionRequest *HostRequest,
  ...
  protected:
    ...
    /** Most recent result information for a session creation attempt, stored here to allow storing error codes for
     * later */
