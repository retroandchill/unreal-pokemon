...
class COMMONUSER_API UAsyncAction_CommonUserInitialize : public UCancellableAsyncAction {
  ...
  public:
    /**
     * Initializes a local player with the common user system, which includes doing platform-specific login and
     * privilege checks. When the process has succeeded or failed, it will broadcast the OnInitializationComplete
     * delegate.
     *
     * @param LocalPlayerIndex	Desired index of ULocalPlayer in Game Instance, 0 will be primary player and 1+ for
     * local multiplayer
