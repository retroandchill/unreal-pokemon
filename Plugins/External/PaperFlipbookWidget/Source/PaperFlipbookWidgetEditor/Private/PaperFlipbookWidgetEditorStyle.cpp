...
#define IMAGE_BRUSH(RelativePath, ...)                                                                                 \
    FSlateImageBrush(FPaperFlipbookWidgetEditorStyle::InContent(RelativePath, ".png"), __VA_ARGS__)
...
FString FPaperFlipbookWidgetEditorStyle::InContent(const FString &RelativePath, const ANSICHAR *Extension) {
    static FString IconsDir =
        IPluginManager::Get().FindPlugin(TEXT("PaperFlipbookWidget"))->GetContentDir() / TEXT("Editor");
    return (IconsDir / RelativePath) + Extension;
}

TSharedPtr<FSlateStyleSet> FPaperFlipbookWidgetEditorStyle::StyleSet = NULL;
TSharedPtr<class ISlateStyle> FPaperFlipbookWidgetEditorStyle::Get() {
    return StyleSet;
}
