...
class PAPERFLIPBOOKWIDGET_API UPaperFlipbookUserWidget : public UWidget {
  ...
  public:
    ...
    /** How to mirror the image in Image mode.  This is normally only used for dynamic image brushes where the source
       texture comes from a hardware device such as a web camera. */
    UPROPERTY(Category = Appearance, EditAnywhere, BlueprintReadWrite)
