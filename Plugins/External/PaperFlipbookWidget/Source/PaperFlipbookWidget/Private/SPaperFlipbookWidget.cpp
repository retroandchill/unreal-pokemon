...
int32 SPaperFlipbookWidget::OnPaint(const FPaintArgs &Args, const FGeometry &AllottedGeometry,
    ...
    if (DefaultBrush.DrawAs != ESlateBrushDrawType::NoDrawType) {
        ...
        const FLinearColor FinalColorAndOpacity(InWidgetStyle.GetColorAndOpacityTint() *
                                                ...
                                                DefaultBrush.GetTint(InWidgetStyle));

        FSlateDrawElement::MakeBox(OutDrawElements, LayerId, AllottedGeometry.ToPaintGeometry(), &DefaultBrush,
...
void SPaperFlipbookWidget::Tick(const FGeometry &AllottedGeometry, const double InCurrentTime,
...
void SPaperFlipbookWidget::SetPlaybackPosition(float NewPosition, bool bFireEvents) {
    ...
    if (bFireEvents) {
        ...
        if (!bReversePlayback) {
            ...
            // Slight hack here.. if playing forwards and reaching the end of the sequence, force it over a little to
            // ensure we fire events actually on the end of the sequence.
