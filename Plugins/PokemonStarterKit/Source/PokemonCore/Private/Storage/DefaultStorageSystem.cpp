...
#include "Ranges/Optional/MapTuple.h"
...
TOptional<FDepositResult> UDefaultStorageSystem::TryDeposit(const TScriptInterface<IPokemon> &Pokemon) {
    ...
    auto Binding = UE::Optionals::MapTuple(&TryDepositToBox, Pokemon);
    // clang-format off
    return Indexes |
                   UE::Ranges::Join |
                   UE::Ranges::ReverseEnumerate<int32>(Boxes) |
                   UE::Ranges::FilterTuple([](int32, const TScriptInterface<IStorageBox> &Box) {
