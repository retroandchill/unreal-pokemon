...
bool UBattleStatusEffectUtils::CanStatusEffectBeInflicted(FName StatusEffectID, const UGameplayAbility *SourceAbility,
    ...
    if (auto &StatusEffect = Target->GetStatusEffect(); StatusEffect.IsSet()) {
        ...
        if (Messages.IsSet()) {
            if (Status.StatusEffectID == StatusEffectID) {
                ...
                auto HasAnotherStatusText = FText::Format(HasOtherStatusFormat, {{"Pkmn", Target->GetNickname()}});
