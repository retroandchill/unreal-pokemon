...
namespace Pokemon::Battle {
    ...
    void CreateRenderComponent(ABattlerSpriteBase& Owner, TObjectPtr<USceneComponent>& Existing, T* Asset) {
        ...
        if constexpr (std::same_as<URenderComponentType, nullptr_t>) {
            UE_LOG(LogBattle, Warning, TEXT("Render component creation for type %s not get implemented"), *T::StaticClass()->GetName());
            if (Existing != nullptr) {
            ...
            if (AsRenderComponent == nullptr) {
                ...
                auto CreatedComponent = Owner.AddComponentByClass(URenderComponentType::StaticClass(), false, Owner.GetBattleSpriteTransform(T::StaticClass()), false);;
...
void ABattlerSpriteBase::SetBattleSprite_Implementation(const FBattleRender &Render) {
    ...
    Render.Visit([this](auto Asset) {
       Pokemon::Battle::CreateRenderComponent(*this, RenderComponent, Asset);
    });
}
