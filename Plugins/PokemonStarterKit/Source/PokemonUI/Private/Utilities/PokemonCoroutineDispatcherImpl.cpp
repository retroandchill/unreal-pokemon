...
UE5Coro::TCoroutine<bool> UPokemonCoroutineDispatcherImpl::GiveItemToPokemon(const UObject* WorldContext,
                                                                             const FItemHandle& Item, const TScriptInterface<IPokemon> Pokemon, int PokemonIndex) const {
    ...
    if (HeldItem.IsSet()) {
        ...
        auto& TextFormat = UStringUtilities::StartsWithVowelText(OldItemName) ? Messages.AlreadyHoldingItemConsonant : Messages.AlreadyHoldingItemVowel;
        co_await Pokemon::UI::DisplayMessage(WorldContext, FText::FormatNamed(TextFormat, "Pkmn", Pokemon->GetNickname(), "Item", OldItemName));
        ...
        co_await Pokemon::UI::DisplayMessage(WorldContext, FText::FormatNamed(Messages.SwappedItemsMessage, TEXT("Pkmn"), Pokemon->GetNickname(), TEXT("Old"), OldItemName, TEXT("New"), NewItem.GetPortionName()));
    } else {
        Pokemon->SetHoldItem(Item);
        ...
        auto& NewItem = FDataManager::GetInstance().GetDataChecked(Item);
        co_await Pokemon::UI::DisplayMessage(WorldContext, FText::FormatNamed(Messages.NowHoldingMessage, TEXT("Pkmn"), Pokemon->GetNickname(), TEXT("Item"), NewItem.GetPortionName()));
    }
