...
void SPokemonStatGraph::CacheNodeData(const FGeometry &AllottedGeometry) {
    CachedStatNames = StatNames.Get();
    const int32 NumNodes = CachedStatNames.Num();
    ...
    const FVector2D TopNormalized = FVector2D(0.0f, DrawRadius / AllottedGeometry.Size.GetMin()).GetRotated(AngleOffset);
...
void SPokemonStatGraph::PaintLines(const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32& DrawLayerId) const {
    //Paint the radial lines
	auto CenterPoint = AllottedGeometry.Size / 2.0f;
