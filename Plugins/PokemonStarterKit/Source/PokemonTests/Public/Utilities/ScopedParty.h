...
namespace Pokemon::Tests {
    ...
    public:
        ...
        explicit FScopedParty(const UObject* WorldContextObject, R&& Pokemon) : Trainer(UTrainerHelpers::GetPlayerCharacter(WorldContextObject).GetObject()), Pokemon(CreatePokemon(WorldContextObject, std::forward<R>(Pokemon))) {
        }
    ...
    private:
        ...
        static TArray<TScriptInterface<IPokemon>> CreatePokemon(const UObject* WorldContextObject, R&& Pokemon) {
            return std::forward<R>(Pokemon) |
                    Retro::Ranges::Views::Transform(Retro::BindFront<&UPartyManagementHelpers::AddPokemonToParty>(WorldContextObject)) |
