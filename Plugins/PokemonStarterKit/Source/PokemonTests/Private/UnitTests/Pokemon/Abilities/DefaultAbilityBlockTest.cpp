...
bool DefaultAbilityBlockTest::RunTest(const FString &Parameters) {
    ...
    UE_CHECK_EQUAL(FName("INNERFOCUS"), Block1->GetAbilityID());
    UE_CHECK_EQUAL(TEXT("Inner Focus"), Block1->GetDisplayName().ToString());
    UE_CHECK_EQUAL(TEXT("The Pokémon is protected from flinching."), Block1->GetAbilityDescription().ToString());
    ...
    UE_CHECK_EQUAL(FName("STEADFAST"), Block2->GetAbilityID());
    UE_CHECK_EQUAL(TEXT("Steadfast"), Block2->GetDisplayName().ToString());
    UE_CHECK_EQUAL(TEXT("Raises Speed each time the Pokémon flinches."), Block2->GetAbilityDescription().ToString());
    ...
    UE_CHECK_EQUAL(FName("JUSTIFIED"), Block3->GetAbilityID());
    UE_CHECK_EQUAL(TEXT("Justified"), Block3->GetDisplayName().ToString());
    UE_CHECK_EQUAL(TEXT("Raises Attack when hit by a Dark-type move."), Block3->GetAbilityDescription().ToString());
    ...
    UE_CHECK_EQUAL(FName(), Block4->GetAbilityID());
    UE_CHECK_EQUAL(TEXT("No Ability"), Block4->GetDisplayName().ToString());
    UE_CHECK_EQUAL(TEXT("This Pokémon has no ability."), Block4->GetAbilityDescription().ToString());
