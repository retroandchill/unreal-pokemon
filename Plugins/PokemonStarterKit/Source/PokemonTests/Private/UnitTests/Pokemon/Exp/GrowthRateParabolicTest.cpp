...
constexpr std::array GParabolicGrowthTable = {
    ...
    1261,   1612,   2035,   2535,   3120,   3798,   4575,   5460,   6458,    7577,   8825,   10208,  11735,
    13411,  15244,  17242,  19411,  21760,  24294,  27021,  29949,  33084,   36435,  40007,  43808,  47846,
    52127,  56660,  61450,  66505,  71833,  77440,  83335,  89523,  96012,   102810, 109923, 117360, 125126,
...
void GrowthRateParabolicTest::GetTests(TArray<FString> &OutBeautifiedNames, TArray<FString> &OutTestCommands) const {
    ...
    for (int i = 0; i < 3; i++) {
        auto Fmt =
            FString::Printf(TEXT("Private.UnitTests.Core.Exp.GrowthRateParabolicTest: Level = 2-99 (Sample %d)"), i + 1);
