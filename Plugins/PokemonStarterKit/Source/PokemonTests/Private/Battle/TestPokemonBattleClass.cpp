...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(TestPokemonBattleClass_ActionSorting, "Unit Tests.Battle.TestPokemonBattleClass.ActionSorting",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(TestPokemonBattleClass_ActionExecution, "Unit Tests.Battle.TestPokemonBattleClass.ActionExecution",
                                 EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool TestPokemonBattleClass_ActionExecution::RunTest(const FString &Parameters) {
    ...
    When(Method(MockAction2, Execute)).AlwaysDo([&bActionMessagesDisplayed] {
    ...
    auto ResultFuture = AsyncThread([&Result] {
        return Result;
    });
