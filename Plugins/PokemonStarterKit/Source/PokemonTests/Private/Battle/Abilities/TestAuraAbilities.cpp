...
bool TestAuraAbilities::RunTest(const FString &Parameters) {
    ...
    auto PlayerPokemon = UnrealInjector::NewInjectedDependency<IPokemon>(World.Get(),
        FPokemonDTO{.Species = TEXT("XERNEAS"), .Level = 50, .Ability = FName(TEXT("FAIRYAURA"))});
    ...
    auto Zygarde = UnrealInjector::NewInjectedDependency<IPokemon>(World.Get(),
        FPokemonDTO{.Species = TEXT("ZYGARDE"), .Level = 50, .Ability = FName(TEXT("AURABREAK"))});
