...
class ENHANCEDSAVEGAME_API UEnhancedSaveGameSubsystem : public UGameInstanceSubsystem {
  ...
  public:
    ...
    /**
     * Creates a new instance of UEnhancedSaveGame and populates it using data from subsystems that implement
     * ISaveableSubsystem.
     *
     ...
     */
    UFUNCTION(BlueprintCallable, BlueprintPure = false, Category = Saving, meta = (AutoCreateRefTerm = SaveTags))
     ...
     */
    UFUNCTION(BlueprintCallable, BlueprintPure = false, Category = Saving, meta = (AutoCreateRefTerm = LoadTags))
    void LoadSaveGame(const UEnhancedSaveGame *SaveGame,
                      const FGameplayTagContainer &LoadTags = FGameplayTagContainer()) const;
