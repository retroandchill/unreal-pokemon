...
bool UPartySwitchHandler::ShouldShow_Implementation(const TScriptInterface<IPartyScreen> &Screen,
                                                    const TScriptInterface<ITrainer> &Trainer, int32 PartyIndex) const {
    return Trainer->GetParty().Num() > 1;
}

void UPartySwitchHandler::Handle_Implementation(const TScriptInterface<IPartyScreen> &Screen,
