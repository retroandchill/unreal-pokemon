...
void SGridMapEditorSettingsWidget::Construct(const FArguments &InArgs, FGridMapEditorUISettings *GridMapUISettings,
    ...
    ChildSlot
        [SNew(SVerticalBox).Visibility(this, &SGridMapEditorSettingsWidget::GetVisibility_SettingsTab)
         // Build Options
         ...
         // Build all
         + SVerticalBox::Slot().AutoHeight().Padding(
               FGridMapStyleSet::StandardPadding)[SNew(SBox).WidthOverride(100.f).HeightOverride(
               20.f)[SNew(SButton)
                         .HAlign(HAlign_Center)
                         .VAlign(VAlign_Center)
                         .OnClicked(this, &SGridMapEditorSettingsWidget::OnRebuildAllTiles)
                         .Text(LOCTEXT("RebuildAllTiles", "Build All Tiles"))
                         .ToolTipText(LOCTEXT("Rebuild All Tiles",
                                              "Recalculates adjacency for all tiles and select the correct mesh"))]]
         // Debug Options
         + SVerticalBox::Slot().AutoHeight().Padding(
               FGridMapStyleSet::StandardPadding)[SNew(SHeader)[SNew(STextBlock)
                      ...
                      .Padding(FGridMapStyleSet::StandardPadding)
                          [SNew(SWrapBox).UseAllottedWidth(true).InnerSlotPadding({6, 5})
                           ...
                           + SWrapBox::Slot()[SNew(SBox).MinDesiredWidth(
                                 150)[SNew(SCheckBox)
                                          .OnCheckStateChanged(
                                              this, &SGridMapEditorSettingsWidget::OnCheckStateChanged_DrawUpdatedTiles)
                                          .IsChecked(this,
                                                     &SGridMapEditorSettingsWidget::GetCheckState_DrawUpdatedTiles)
                                          .ToolTipText(LOCTEXT("DebugOption_DrawUpdatedTiles",
                                                               "Whether to draw debug info for updated tiles"))
                                              [SNew(STextBlock)
                                                   .Text(LOCTEXT("DebugOption_DrawUpdatedTiles_Label", "Show Updated "
                                                                                                       "Tiles"))
                                                   .Font(FGridMapStyleSet::StandardFont)]]]]]];
}
