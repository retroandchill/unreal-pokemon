...
BEGIN_DEFINE_SPEC(FOptionalPtrSpec, "OptionalPtr.Unit", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::EditorContext)
		...
		Describe("when given a wrapped UObject pointer", [this]()
		{
			Describe("when valid", [this]()
			{
				...
				It("should map the overloaded static function 2, set reference bool and return the value", [this]()
				{
					...
					bool executed = false;
					auto testing_obj = TOptionalPtr<UMockUObject>(wrapped_obj).
						Map(static_cast<SimpleObject*(*)(MockObject*, bool&)>(&MockObject::OverloadedStaticFunction), executed);
					TestTrue("", testing_obj.IsSet());
					TestTrue("", executed);
					TestTrue("", std::is_same<decltype(testing_obj), TOptionalPtr<SimpleObject>>::value);
...
BEGIN_DEFINE_SPEC(FOptionalPtrPerformanceSpec, "OptionalPtr.Performance", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::EditorContext)
