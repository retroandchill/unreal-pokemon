...
namespace UE::Ranges {
    ...
    class TClassViewIterator {
      public:
        ...
        explicit TClassViewIterator(const TSharedRef<TArray<TSubclassOf<T>>> &Objects, SizeType Index = 0)
      ...
      public:
        explicit TClassView(bool bIncludeAbstract = false, bool bIncludeDerivedClasses = true,
            ...
            auto ClassCheck = [this, bIncludeAbstract](UObject *Object) {
                ...
                if (!AsClass->IsChildOf<T>() || (!bIncludeAbstract && AsClass->HasAnyClassFlags(CLASS_Abstract)) ||
                    !IsInstantiableClass(AsClass)) {
                    return;
                }

                ClassArray->Emplace(AsClass);
            };
            ForEachObjectOfClass(UClass::StaticClass(), ClassCheck, bIncludeDerivedClasses, ExclusionFlags,
                                 ExclusionInternalFlags);
        }

        explicit TClassView(TArray<TSubclassOf<T>> &&ClassArray)
            : ClassArray(MakeShared<TArray<TSubclassOf<T>>>(std::move(ClassArray))) {
        }
