    ...
    struct TConstructInvoker {
        ...
        constexpr auto operator()(R&& Range, A&&... Args) const {
            if constexpr (sizeof...(A) > 0) {
                return Range | ranges::views::transform(ranges::bind_back([]<typename... B>(B&&... BoundArgs) {
                    return T(Forward<B>(BoundArgs)...);
                }, Forward<A>(Args)...));
            } else {
                return Range | ranges::views::transform([]<typename... B>(B&&... BoundArgs) {
                    return T(Forward<B>(BoundArgs)...);
                });
            }
        }
