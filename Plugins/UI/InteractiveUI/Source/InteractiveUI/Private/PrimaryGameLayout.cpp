...
void UPrimaryGameLayout::SetIsDormant(const bool bNewIsDormant)
{
    if (bIsDormant != bNewIsDormant)
    {
        ...
        UE_LOG(LogInteractiveUI, Display, TEXT("%s PrimaryGameLayout Dormancy changed for [%d] from [%s] to [%s]"),
                                                    ...
                                                    const bool bIsTransitioning)
{
    if (bIsTransitioning)
    {
        const FName SuspendToken =
            UCommonUIExtensions::SuspendInputForPlayer(GetOwningLocalPlayer(), TEXT("GlobalStackTransition"));
