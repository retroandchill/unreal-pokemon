...
namespace Simple2D
{
                 ...
                 [SNew(SScrollBox)
                  ...
                  SScrollBox::Slot()
                      [SNew(SOverlay)
                       ...
                       + SOverlay::Slot().VAlign(VAlign_Fill)[BackgroundPerFrameSlices.ToSharedRef()]

                       // Flipbook header and track
                       + SOverlay::Slot()[SNew(SVerticalBox)

                                          + SVerticalBox::Slot().AutoHeight().Padding(0, 0, 0,
                                                                                      2)[TimelineHeader.ToSharedRef()]

                                          + SVerticalBox::Slot().AutoHeight()[SNew(SBox).HeightOverride(
                                                Flipbooks::FrameHeight)[TimelineTrack.ToSharedRef()]]]
                       ...
                       + SOverlay::Slot()
                             ...
                             .HAlign(HAlign_Center)
                                 [SNew(STextBlock)
                                      ...
                                      .Text(NSLOCTEXT("Simple2D", "EmptyTimelineInstruction",
                                                      "Right-click here or drop in sprites to add key frames"))]]]];
                                           ...
                                           int32 LayerId, const FWidgetStyle &InWidgetStyle, bool bParentEnabled) const
    {
        ...
        const float CurrentTimeXPos =
            CurrentTimeSecs / TotalTimeSecs * SlateTotalDistance - AnimationScrollBarPosition +
            FMath::Clamp((AllottedGeometry.GetLocalSize().X + AnimationScrollBarPosition) - SlateTotalDistance, 0.0f,
                         AnimationScrollBarPosition);
    ...
    FReply SSimpleFlipbookTimeline::OnMouseWheel(const FGeometry &MyGeometry, const FPointerEvent &MouseEvent)
    {
        if (MouseEvent.IsControlDown())
        {
            ...
            const float NewUnitsPerFrame =
                FMath::Max(MinFrameSize, SlateUnitsPerFrame * (1.0f + Direction * DirectionScale));
    ...
    void SSimpleFlipbookTimeline::RebuildPerFrameBG()
    {
        ...
        for (int32 FrameIndex = 0; FrameIndex < NumFramesFromLastRebuild; ++FrameIndex)
        {
            ...
            BackgroundPerFrameSlices->AddSlot().AutoWidth()[SNew(SBox).WidthOverride(
                SlateUnitsPerFrame)[SNew(SColorBlock).Color(BackgroundColorForFrameIndex)]];
        }
    }
