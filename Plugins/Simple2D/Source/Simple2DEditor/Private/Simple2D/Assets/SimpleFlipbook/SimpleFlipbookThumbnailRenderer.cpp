...
void USimpleFlipbookThumbnailRenderer::Draw(UObject *Object, int32 X, int32 Y, uint32 Width, uint32 Height,
    ...
    if (auto* Flipbook = Cast<USimpleFlipbook>(Object); Flipbook != nullptr)
    {
        ...
        if (const int32 KeyFrameIndex = Flipbook->GetKeyFrameIndexAtTime(PlayTime); Flipbook->IsValidKeyFrameIndex(KeyFrameIndex)) {
...
void USimpleFlipbookThumbnailRenderer::DrawFrame(USimpleFlipbook* Flipbook, int32 Index, int32 X, int32 Y,
                                                 uint32 Width, uint32 Height, FRenderTarget *, FCanvas *Canvas) {
	...
	if (SourceTexture != nullptr)
	{
	    ...
	    FVector2D ActualSize(static_cast<double>(SourceTexture->GetSizeX()) / Flipbook->GetColumns(), static_cast<double>(SourceTexture->GetSizeY()) / Flipbook->GetRows());
