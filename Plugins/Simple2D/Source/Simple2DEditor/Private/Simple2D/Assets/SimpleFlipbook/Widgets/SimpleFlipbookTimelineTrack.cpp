...
namespace Simple2D {
    ...
    TSharedRef<FKeyFrameDragDropOp> FKeyFrameDragDropOp::New(int32 InWidth, USimpleFlipbook *InFlipbook,
        ...
        auto Operation = MakeShared<FKeyFrameDragDropOp>(FInitTag(), InFlipbook->GetKeyFrameChecked(InFrameIndex), InFrameIndex, InFlipbook, InWidth);
        Operation->Construct();
    ...
    FReply SSimpleFlipbookKeyframeWidget::OnDragDetected(const FGeometry &MyGeometry, const FPointerEvent &MouseEvent) {
        ...
        {
            if (auto* Flipbook = FlipbookBeingEdited.Get(); (Flipbook != nullptr) && Flipbook->IsValidKeyFrameIndex(FrameIndex))
            {
    ...
    FText SSimpleFlipbookKeyframeWidget::GetKeyframeTooltip() const {
        if (auto KeyFrame = GetKeyFrameData(); KeyFrame.IsSet())
        {
            const FText SpriteLine = (KeyFrame->Index != INDEX_NONE) ? FText::FormatOrdered(NSLOCTEXT("Simple2D", "SpriteFrame", "Frame {0}"), KeyFrame->Index) : NSLOCTEXT("Simple2D", "NoSprite", "(none)");

            const FText FramesText = (KeyFrame->FrameRun == 1) ? NSLOCTEXT("Simple2D", "SingularFrames", "frame") : NSLOCTEXT("Simple2D", "PluralFrames", "frames");
    ...
    TSharedRef<SWidget> SSimpleFlipbookKeyframeWidget::GenerateContextMenu() {
        ...
        FMenuBuilder MenuBuilder(true, CommandList);
        {
            ...
            const FText KeyframeSectionTitle = FText::Format(NSLOCTEXT("Simple2D", "KeyframeXActionsSectionHeader", "Keyframe #{0} Actions"), FText::AsNumber(FrameIndex, &NoCommas));
    ...
    void SSimpleFlipbookTimelineTrack::Construct(const FArguments &InArgs, const TSharedPtr<FUICommandList> &InCommandList) {
