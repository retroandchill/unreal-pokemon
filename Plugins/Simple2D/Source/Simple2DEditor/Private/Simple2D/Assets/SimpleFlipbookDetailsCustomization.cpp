...
namespace Simple2D {
    ...
    void FSimpleFlipbookDetailsCustomization::CustomizeDetails(IDetailLayoutBuilder &DetailBuilder) {
        ...
        auto CustomPivotPointProperty = DetailBuilder.GetProperty(
            GET_MEMBER_NAME_CHECKED(USimpleFlipbook, CustomPivotPoint));
        auto CustomPivotPointVisibility = TAttribute<EVisibility>::Create(
            TAttribute<EVisibility>::FGetter::CreateStatic(&FSimpleFlipbookDetailsCustomization::GetCustomPivotVisibility, PivotModeProperty.ToSharedPtr()));
    ...
    void FSimpleFlipbookDetailsCustomization::ExtractSpriteSamplerLabels(FText &SourceTextureOverrideLabel, class UMaterial *DefaultMaterial) {
        ...
        for (const UMaterialExpressionSpriteTextureSampler *Sampler : SpriteSamplerExpressions) {
            ...
            if (Sampler->bSampleAdditionalTextures) {
                AdditionalTextureLabels.FindOrAdd(Sampler->AdditionalSlotIndex) = Sampler->
                    SlotDisplayName;
            } else {
    ...
    void FSimpleFlipbookDetailsCustomization::BuildTextureSection(IDetailCategoryBuilder &SpriteCategory,
        ...
        auto DefaultMaterialProperty = DetailLayout.GetProperty(
            GET_MEMBER_NAME_CHECKED(USimpleFlipbook, DefaultMaterial));

        FText SourceTextureOverrideLabel;
        if (UObject *DefaultMaterialAsObject; DefaultMaterialProperty->GetValue(DefaultMaterialAsObject) ==
        ...
        auto AdditionalSourceTexturesProperty = DetailLayout.GetProperty(GET_MEMBER_NAME_CHECKED(USimpleFlipbook, AdditionalSourceTextures));
        auto AdditionalSourceTexturesBuilder = MakeShared<FDetailArrayBuilder>(AdditionalSourceTexturesProperty);
        AdditionalSourceTexturesBuilder->OnGenerateArrayElementWidget(
    ...
    void FSimpleFlipbookDetailsCustomization::GenerateAdditionalTextureWidget(
        ...
        auto SlotDesc = FText::Format(NSLOCTEXT("Simple2D", "AdditionalTextureSlotIndex", "Slot #{0}"), FText::AsNumber(ArrayIndex, &NoCommas));
    ...
    EVisibility FSimpleFlipbookDetailsCustomization::GetCustomPivotVisibility(
        TSharedPtr<IPropertyHandle> Property) {
        if (Property.IsValid()) {
            uint8 ValueAsByte;
            if (auto Result = Property->GetValue(ValueAsByte); Result == FPropertyAccess::Success) {
                return static_cast<ESpritePivotMode::Type>(ValueAsByte) == ESpritePivotMode::Custom ? EVisibility::Visible : EVisibility::Collapsed;
            }
