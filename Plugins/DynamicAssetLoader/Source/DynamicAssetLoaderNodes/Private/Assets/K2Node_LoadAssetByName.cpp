...
static const FName Load_AssetNotFoundPinName = "AssetNotFound";
...
void UK2Node_LoadAssetByName::AllocateDefaultPins() {
    ...
    CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Exec, Load_AssetNotFoundPinName);
...
void UK2Node_LoadAssetByName::ExpandNode(FKismetCompilerContext &CompilerContext, UEdGraph *SourceGraph) {
    ...
    if (auto &ClassType = GetDefault<UAssetLoadingSettings>()->AssetClasses.FindChecked(AssetKey);
        ...
        auto CallCastNode = CompilerContext.SpawnIntermediateNode<UK2Node_CastObjectToVariant>(this, SourceGraph);
...
UEdGraphPin *UK2Node_LoadAssetByName::GetAssetNotFoundPin() const {
    UEdGraphPin *Pin = FindPinChecked(Load_AssetNotFoundPinName);
    check(Pin->Direction == EGPD_Output)
    return Pin;
}
