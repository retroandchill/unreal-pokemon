...
namespace Retro {
    ...
    UE5Coro::TCoroutine<> BindToDelegateDispatch(D &Delegate, F &&Functor) {
        ...
        if constexpr (Delegates::UnicastDelegate<D>) {
            Delegate.BindLamba([&State](auto &&...) { State->EmplaceResult(0); });
        } else {
            Delegate.AddLambda([&State](auto &&...) { State->EmplaceResult(0); });
        }
    ...
    UE5Coro::TCoroutine<Delegates::TDelegateTuple<D>> BindToDelegateDispatch(D &Delegate, F &&Functor) {
        ...
        if constexpr (Delegates::UnicastDelegate<D>) {
            Delegate.BindLamba(
                [&State]<typename... A>(A &&...Args) { State->EmplaceResult(std::forward<A>(Args)...); });
        } else {
            Delegate.AddLambda(
                [&State]<typename... A>(A &&...Args) { State->EmplaceResult(std::forward<A>(Args)...); });
        }
