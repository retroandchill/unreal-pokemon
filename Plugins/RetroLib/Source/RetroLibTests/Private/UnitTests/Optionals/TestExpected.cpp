...
TEST_CASE_NAMED(FTestExpectedOperations, "Unit Tests::RetroLib::Optionals::Expected::Operations",
                "[RetroLib][Optionals]") {
...
TEST_CASE_NAMED(FTestExpectedTransform, "Unit Tests::RetroLib::Optionals::Expected::Pipes::Transform",
                "[RetroLib][Optionals]") {
    ...
    SECTION("Can transform a void expected, getting a simple supplier") {
        auto Transformed = Retro::TExpected<void, std::string>() | Retro::Optionals::Transform([] { return 5; });
        REQUIRE(Transformed.IsSet());
        CHECK(Transformed == 5);
    }
...
TEST_CASE_NAMED(FTestExpectedFilter, "Unit Tests::RetroLib::Optionals::Expected::Pipes::Filter",
                "[RetroLib][Optionals]") {
    SECTION("Can filter an expected, default constructing the error") {
        Retro::TExpected<float, std::string> TestValue = 1.5;
        auto ValidFilter = TestValue | Retro::Optionals::Filter([](float V) { return V > 1.f; });
        REQUIRE(ValidFilter.IsSet());
        CHECK(ValidFilter == TestValue);

        auto InvalidFilter = TestValue | Retro::Optionals::Filter([](float V) { return V < 1.f; });
    ...
    SECTION("Can supply a functor to create the filter") {
        auto Filtered = Retro::TExpected<float, std::string>(1.5) |
                        Retro::Optionals::Filter([](float V) { return V < 1.f; },
                                                 [] { return "Expected value to be less than 1.5"; });
        REQUIRE(!Filtered.IsSet());
        CHECK(Filtered.GetError() == "Expected value to be less than 1.5");
    }
}

TEST_CASE_NAMED(FTestExpectedIfPresent, "Unit Tests::RetroLib::Optionals::Expected::Pipes::IfPresent",
                "[RetroLib][Optionals]") {
...
TEST_CASE_NAMED(FTestExpectedIfPresentOrElse, "Unit Tests::RetroLib::Optionals::Expected::Pipes::IfPresentOrElse",
                "[RetroLib][Optionals]") {
...
TEST_CASE_NAMED(FTestExpectedAndThen, "Unit Tests::RetroLib::Optionals::Expected::Pipes::AndThen",
                "[RetroLib][Optionals]") {
    SECTION("Can convert an expected into an optional") {
        Retro::TExpected<int, std::string> TestValue(14);
        auto AsOptional = TestValue | Retro::Optionals::AndThen([](int V) { return std::optional(V * 2); });
        CHECK(AsOptional == 28)
    }
    ...
    SECTION("Can convert an expected to another expected") {
        Retro::TExpected<int, std::string> TestValue(14);
        auto AsExpected =
            TestValue | Retro::Optionals::AndThen([](int V) { return Retro::TExpected<int, std::string_view>(V * 2); });
        CHECK(AsExpected == 28)
    }
    ...
    SECTION("Can convert an optional to an expected, constructing the error type if needed") {
        ...
        auto AsExpected =
            Optional | Retro::Optionals::AndThen([](int V) { return Retro::TExpected<int, std::string_view>(V * 2); },
                                                 "Unused error");
...
TEST_CASE_NAMED(FTestExpectedTo, "Unit Tests::RetroLib::Optionals::Expected::Pipes::To", "[RetroLib][Optionals]") {
    SECTION("Can convert an expected into an optional") {
        Retro::TExpected<int, std::string> TestValue(14);
        auto AsOptional = TestValue | Retro::Optionals::To<std::optional>();
        CHECK(AsOptional == 14)
    }

    SECTION("Can convert an expected to another expected") {
        Retro::TExpected<int, std::string> TestValue(14);
        auto AsExpected = TestValue | Retro::Optionals::To<Retro::TExpected<int, std::string_view>>();
        CHECK(AsExpected == 14)
    }

    SECTION("Can convert an expected to another expected keeping the error") {
        Retro::TExpected<int, std::string> TestValue(Retro::Unexpect, "Invalid value");
        auto AsExpected = TestValue | Retro::Optionals::To<Retro::TExpected, std::string_view>();
        REQUIRE(!AsExpected.IsSet())
        CHECK(AsExpected.GetError() == "Invalid value");
    }
    ...
    SECTION("Can convert an optional to an expected, constructing the error type if needed") {
        std::optional Optional = 4;
        auto AsExpected = Optional | Retro::Optionals::To<Retro::TExpected<int, std::string_view>>("Unused error");
        CHECK(AsExpected.IsSet())
    }
    ...
    SECTION("Can convert an optional to an expected, constructing the error type on demand") {
        std::optional<int> Optional;
        auto AsExpected =
            Optional | Retro::Optionals::To<Retro::TExpected, std::string>([] { return "Constructed error"; });
        REQUIRE(!AsExpected.IsSet())
        CHECK(AsExpected.GetError() == "Constructed error");
    }
}
...
TEST_CASE_NAMED(FTestTransformError, "Unit Tests::RetroLib::Optionals::TransformError", "[RetroLib][Optionals]") {
    SECTION("Can transform the error of an expected") {
        Retro::TExpected<int, std::string> TestValue(Retro::Unexpect, "Invalid value");
        auto Transformed = TestValue | Retro::Optionals::TransformError([](const std::string &V) { return V.size(); });
        REQUIRE(!Transformed.IsSet())
