...
bool GenderRatioTest::RunTest(const FString &Parameters) {
    ...
    auto Pokemon2 = UnrealInjector::NewInjectedDependency<IPokemon>(World, FPokemonDTO{.Species = "LUCARIO", .PersonalityValue = 0x39593A01});
    ASSERT_EQUAL(Pokemon2->GetGender(), Female);

    auto Pokemon3 = UnrealInjector::NewInjectedDependency<IPokemon>(World, FPokemonDTO{.Species = "OSHAWOTT", .PersonalityValue = 0x39593AA3});
