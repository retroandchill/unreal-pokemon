...
TScriptInterface<ITrainer> UTrainerStub::Initialize(FName NewTrainerType, FText NewTrainerName) {
    ...
    AddPokemonToParty(UConstructionUtilities::CreateForeignPokemon(
        {.Species = "RIOLU", .Level = 10, .Shiny = true, .Item = FName("BLACKBELT"), .ObtainMethod = EObtainMethod::FatefulEncounter}, this));
    ...
    AddPokemonToParty(UConstructionUtilities::CreateForeignPokemon(
        {.Species = "FRILLISH", .Level = 10, .Gender = EPokemonGender::Male, .MetLocation = FText::FromStringView(TEXT("Test Map"))}, this));
    AddPokemonToParty(UConstructionUtilities::CreateForeignPokemon(
