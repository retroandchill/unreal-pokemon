...
{
    explicit FScopedArrayDeleter(FScriptArrayHelper &Helper) : Helper(Helper)
    {
    }
                                               ...
                                               FString &ErrorMessage) const
{
    ...
    TArray<TSharedPtr<FJsonValue>> JsonValues;
    if (const auto Reader = TJsonReaderFactory<>::Create(MoveTemp(JsonString));
    ...
    for (const auto &JsonValue : JsonValues)
    {
        ...
        if (const int32 NewIndex = ArrayHelper.AddValue(); !FJsonObjectConverter::JsonObjectToUStruct(
                JsonObject.ToSharedRef(), StructType, ArrayHelper.GetElementPtr(NewIndex)))
        {
            ErrorMessage = TEXT("Failed to deserialize entry");
            return false;
        }
